...
- **🐾 選單列用量監看**：常駐右上角顯示 Claude Code + Codex 的 5 小時 / 7 天配額，數字與進度條同色（黃 / 綠 / 紅），掃一眼就知道警示級別。點開看 Session、Weekly、各專案用量與今日成本估算。
- **🔄 進度管家**：開新 Claude Code 對話時，自動把上次的進度（你上次的請求、完成的 commit、未完成待辦）交給 AI，不用再重講一次。純本地、零 API，預設關閉。詳見[官方介紹頁](https://aqua5230.github.io/usage/#resume)。
- **🎨 9 款視覺面板**：預設、駭客任務、視窗 95、復古報紙、雲圖觀測、午夜水族箱、稜鏡街機、黑洞視界、世界盃 2026，一鍵切換。
- **📊 HTML 深度報告**：每日 / 週 / 月 token 與成本走勢、各專案排名、Top 模型分布，可一鍵分享給同事（內建隱藏專案名稱選項）。
- **🌍 5 語言 i18n**：繁中 / 簡中 / 英 / 日 / 韓，自動跟隨系統語言。
