...
聊天版（網頁、桌面 App）會記得你的對話，關掉再開全都還在；但 Claude Code 的 CLI（命令列）不會——每次開新對話都得重講一次「我們上次做到哪」。**進度管家**（選單裡叫「接著上次做」）把這個能力補給 CLI：

- 開新 Claude Code 對話時，自動把上次的進度（你上次的請求、完成的 commit、未完成的待辦）交給 AI，不用再重講一次
- **純本地、零 API**：只讀你電腦上 Claude Code 自己留下的紀錄檔，不連網、不呼叫任何 API
- 選單一個開關，**預設關閉**（opt-in），開了才會裝那個事件觸發點（SessionStart hook）

詳細介紹見[官方介紹頁的進度管家段落](https://aqua5230.github.io/usage/#resume)。
...
| 我是… | 怎麼用 |
|-------|--------|
| 一般使用者，想直接用 | [Homebrew 安裝](#homebrew-安裝推薦) 或 [下載 App](#下載現成-app) |
| 開發者，想從原始碼跑 | [建環境](#建環境) |
| 只想先看看 UI 長什麼樣 | [預覽模式](#想先看看-ui-長什麼樣預覽模式) |

## Homebrew 安裝（推薦）

最簡單的安裝方式，一鍵裝好、日後 `brew upgrade` 自動更新：
