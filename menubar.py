...
def _setup_fsevents(delegate: Any) -> Any:
    """Start FSEventStream watching usage data directories; returns stream handle or None."""
    ...
    try:
        watch_paths = [
            path
            for path in (Path.home() / ".claude", Path.home() / ".codex" / "sessions")
            if path.is_dir()
        ]
        if not watch_paths:
            return None
...
def _generate_analysis_report(period: str = "last30", language: str | None = None) -> str:
